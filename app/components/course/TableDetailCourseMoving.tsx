...
import { detailCourseMoving } from '../../constants/course.cont';
...
type Course = (typeof detailCourseMoving)[0];

export default function TableDetailCourseMoving() {
  ...
  const sortedItems = React.useMemo(() => {
    return detailCourseMoving.sort((a: Course, b: Course) => {
  ...
  const renderCell = useCallback((course: Course, columnKey: Key) => {
    ...
    switch (columnKey) {
      ...
      case 'loadingConsolidationStore':
        return (
          <div className='flex items-center gap-2'>
            <span className='truncate'>{course.loadingConsolidationStore}</span>
            <PencilEdit />
          </div>
        );
      ...
      case 'loadingDestConference':
        return (
          <div className='flex items-center gap-2'>
            <span>{course.loadingDestConference}</span>
            <PencilEdit />
          </div>
        );
      case 'unloadingShop':
        return (
          <div className='flex items-center gap-2'>
            <span>{course.unloadingShop}</span>
            <PencilEdit />
          </div>
        );
      case 'loadingDate':
